...
from web.forms import AuthorForm, AuthorUpdateForm
...
def index():
    authors = Author.query.order_by(Author.name).paginate(page=1, per_page=app.config['ITEMS_PER_PAGE'], error_out=False)
...
def index_author_pages(page_num):
    authors = Author.query.order_by(Author.name).paginate(page=page_num, per_page=app.config['ITEMS_PER_PAGE'], error_out=False)
    return render_template('authors/index.html', authors=authors)
